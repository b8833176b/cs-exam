...
public class ImportExcelUtil {
    ...
    private static String excelfont = "微软雅黑";
	  ...
	  * @throws IOException 
	  */
	public Workbook getWorkbook(InputStream inStr,String fileName) throws FileFormatException, IOException {
		Workbook wb=null;
		String fileType=fileName.substring(fileName.lastIndexOf("."));
		if(excel2003L.equals(fileType)) {
			wb = new HSSFWorkbook(inStr);
		}else if(excel2007U.equals(fileType)) { //对于此格式今后处理
		 
		}else {
			throw new FileFormatException("文件格式有误！");
		}
		return wb;
	}
	        ...
	        if(widths==null){  
	          ...
	          if(ds_format==null){  
	           ...
	           //设置文件名  
	            ...
	            //创建一个sheet  
	            HSSFSheet  sheet = wb.createSheet((!StringUtil.isEmpty(sheetName))?sheetName:"excel");  
	            //创建表头，如果没有跳过  
	            int headerrow = 0;  
	            if(headers!=null){  
	            ...
	            //表格主体  解析list  
	            if(data != null){  
	                ArrayList<Object> styleList = new ArrayList<>();  
